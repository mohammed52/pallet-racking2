...
export function getUprightSpecsAndCost(rackingRequirements) {
	...
	for (var i = 0; i < UprightWeightTable.length; i++) {
		if (
			(UprightWeightTable[i].maxCapacityPerLevel >= maxLevelCapacity &&
				UprightWeightTable[i].maxCapacityPerRack >= maxRackCapacity) ||
			i == UprightWeightTable.length - 1
		) {
	...
	// const UOM = "kgs"
	const unitWeight =
		rackingRequirements.frame.frameHeight * selectedUpright.weightPerFoot;
	...
	if (MAPLOG)
		console.log("uprightSpecsAndTotalWeight: ", uprightSpecsAndTotalWeight);
